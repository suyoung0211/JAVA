...
public class C10MemberCompare {
    public static void main(String[] args) {
        ...
        int diff = m1.compareTo(m3); // this 는 m1, 다른 하나 m3
        System.out.println("m1 과 m3 name 비교 : " + diff);

        diff = m3.compareTo(m1); // this 는 m3, 다른 하나 m1
        System.out.println("m3 과 m1 name 비교 : " + diff);
...
class Member implements Cloneable, Comparable<Member> {
    ...
    protected Member clone() throws CloneNotSupportedException { // 예외 처리 위임
        ...
        // throws 뒤의 예외는 Checked 예외 -> try ~ catch -> 다른 곳으로 위임
    }

    // Comparable 인터페이스 재정의 - Member 객체의 어떤 속성값을 비교할 것인지
    @Override
