...
public class ProductManagerApp {
            ...
            try {
                switch (menu.toUpperCase()) {
                    ...
                    case "U":
                        ...
                        double price = Double.parseDouble(System.console().readLine("\t판매 가격 입력 >>> "));
                        if (confirm("포인트 변경")) {
                            list.get(index).setPrice(price);
                            System.out.println("\t업데이트 결과 : " + price);
                        } else {
    ...
    private static void initProduct(List<Product> list) {
    ...
    private static void addProduct(List<Product> list) {
        int id = new Random().nextInt(8889) + 1111; // 수정 후
        // int id = (int) (Math.random() * 8889 + 1111); // 수정 전
