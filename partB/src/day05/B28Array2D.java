...
public class B28Array2D {
    public static void main(String[] args) {
        ...
        // 2차원 배열의 요소인 배열들은 크기가 각각 다를 수 있습니다.
        ...
        System.out.println(matrix[0] + ", "
                ...
                + matrix[0].length);
        // [I@372f7a8d -> matrix[0] 배열의 참조값
        // [1, 2, 3, 4] -> matrix[0] 배열의 데이터들
        // 4 -> matrix[0] 배열의 크기

        System.out.println("인덱스 2번 배열의 1번 값을 출력 : " + matrix[2][1]);

        for (int i = 0; i < matrix.length; i++) { // 배열의 위치(인덱스) 0, 1, 2
            for (int j = 0; j < matrix[i].length; j++) { // 배열 안에서의 인덱스 0, 1, 2, 3
                System.out.println(
                        String.format("matrix[%d][%d] = [%d]", i, j, matrix[i][j]));
            }
        }
