...
public class B19MyArrayMethod {
    public static void main(String[] args) {
        ...
        // changeArrayValue() 메소드 실행
        changeArrayValue(scores, 2, 69);
        System.out.println("메소드 실행 후 : " + Arrays.toString(scores));

        int[] score2 = { 99, 67, 100 };

        // sumOfArrayValue() 메소드 출력
        System.out.println("sumOfArrayValue(score) : " + sumOfArrayValue(scores));
        System.out.println("sumOfArrayValue(score2) : " + sumOfArrayValue(score2));

        // mergeOfArray() 메소드 출력
        int[] mergeArr = mergeOfArray(scores, score2); // 리턴이 배열의 참조값 mergeArr 변수에 저장
    ...
    private static int[] mergeOfArray(int[] arr1, int[] arr2) {
        ...
        // arr2 을 target 배열 reult 에 복사
        // 첫번째 반복문이 끝나면 result 인덱스 변수 i=arr1.length 부터 시작
